        ...
        if(sc){
            if(sc.scrollLeft == 0 && leftB){
                sc.scrollBy({
                    left:sc.scrollWidth,
                    behavior: 'smooth',
            })}else if (sc.scrollLeft >= sc.scrollWidth - sc.clientWidth - 1 && !leftB){
                sc.scrollBy({
                    left: -sc.scrollWidth,
                    behavior: 'smooth',
            })}else{
                sc.scrollBy({
                    left: leftB ? -sc.scrollWidth / lenght*2 : sc.scrollWidth / lenght*2,
                    behavior: 'smooth',
                })
            }
        }
    ...
    <div>
        <h2 className="text-center text-4xl p-5 font-main">{name}</h2>
            <div className="flex items-center lg:mx-5">
                ...
                <div ref={scrollRef} className="flex h-auto w-auto overflow-auto scrollbar-hide">
                    {list.map((lang, index) => (
